...
public:
	...
	/// - `x` : The other array.
	template<size_t capacity_x_>
	bool equal(const hxarray<T_, capacity_x_>& x_) const hxattr_nodiscard;

	/// Returns true if the array is empty.
	/// Callers must check the return value instead of discarding it.
	bool empty(void) const hxattr_nodiscard { return m_end_ == this->data(); }
	...
	bool full(void) const hxattr_nodiscard { return m_end_ == this->data() + this->capacity(); }

	/// Appends `size` elements generated by invoking `fn` repeatedly.
	/// - `size` : Number of elements to append.
	/// - `fn` : Functor returning the elements to append.
	template<typename functor_t_>
	void generate_n(size_t size_, functor_t_&& fn_);
	...
	/// - `x` : The other array.
	template<size_t capacity_x_>
	bool less(const hxarray<T_, capacity_x_>& x_) const hxattr_nodiscard;
...
template<hxarray_concept_ T_, size_t capacity_>
template<typename functor_t_>
void hxarray<T_, capacity_>::generate_n(size_t size_, functor_t_&& fn_) {
	while(size_--) {
		::new(this->push_back_unconstructed_()) T_(hxforward<functor_t_>(fn_)());
	}
}
