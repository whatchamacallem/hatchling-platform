...
/// When `multi_t` is `false`, `insert_node` returns the existing node on a key
/// collision without inserting the new node. When `multi_t` is `true`, duplicate
/// keys are always inserted.
